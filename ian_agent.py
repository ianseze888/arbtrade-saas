...
def build_ian_ws_prompt(user_id: str, criteria: dict) -> str:
    ...
    return (
        ...
        "- Mark BUY if: ROI over " + str(min_roi) + "%, BSR under " + str(max_bsr) + ", under " + str(max_sellers) + " sellers, Amazon NOT on listing\n"
        "- Mark WATCH if: ROI 20-" + str(min_roi) + "% OR moderate competition\n"
        "- Mark PASS if: ROI under 20% OR Amazon on listing OR over " + str(max_sellers) + " sellers\n"
        "- AIM for at least 4 BUY recommendations in your 6 leads\n\n"
        "CRITERIA:\n"
        ...
        "BUY = ROI over 25%, BSR under 100k, under 10 sellers, no Amazon on listing\n"
        "WATCH = ROI 15-25% or moderate competition\n"
        "PASS = low ROI, Amazon on listing, or too competitive\n"
        "AIM for at least 4 BUY recommendations per batch\n"
...
def build_ian_oa_prompt(user_id: str, criteria: dict) -> str:
    ...
    return (
        ...
        "RECOMMENDATION RULES (CRITICAL):\n"
        "- Mark BUY if: ROI over " + str(min_roi) + "%, product clearly profitable\n"
        "- Mark WATCH if: ROI 20-" + str(min_roi) + "% OR price may change\n"
        "- Mark PASS if: ROI under 20% OR too risky\n"
        "- AIM for at least 3 BUY recommendations in your 6 OA leads\n\n"
        "CRITERIA:\n"
